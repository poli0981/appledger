...
| Pipe security | `AppLedger.Infrastructure.Tests` | CI (non-admin) | the descriptor is built as ADR-17 specifies, and a **real pipe reads back the Medium mandatory label** — the one assertion that distinguishes a working descriptor from the managed `PipeSecurity` route, which drops the label without a word (`24_ADR.md` §Findings, 2026-08-28). Also peer verification over a real pipe, and that an unreadable peer is refused rather than trusted. Runs unelevated because writing a label at or below your own integrity needs no privilege |
| Pipe security (elevated) | `AppLedger.Infrastructure.Tests`, **`Category=Admin`** | elevated dev box only | that a *High-integrity* process applies the same descriptor, and that the pipe does not end up carrying a High label instead. The connect that matters — Medium-IL client to High-IL server — is manual: one test process cannot be both integrity levels (`tests/MANUAL_CHECKLIST.md`) |
...
## Manual matrix (per release, `Category=Manual` checklist in `tests/MANUAL_CHECKLIST.md`)
