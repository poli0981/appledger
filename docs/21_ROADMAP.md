...
| **v0.1 — Skeleton** — **Core half done** | ✔ solution + CPM + analyzers + self-contained CI; ✔ Core models, `PolicyGuard` rules and tier table, rollup math with golden tests, strict catalog parser and validator, minisign **parsing**, PSL/eTLD+1, formatters and redactors; ✔ 12 S2 fixtures authored.<br>**Remaining:** Infrastructure smoke tests — ntdll poller, known folders, full canonicalization (8.3 + `GetFinalPathNameByHandleW`), PE/Authenticode, SQLite migrations, and minisign **verification** behind `ICatalogVerifier` | `dotnet test` green on CI; Core coverage ≥ 80 % — **met: 92.1 %** |
