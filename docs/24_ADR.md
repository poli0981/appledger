...
| 2026-08-27 | 8.3 short-name generation is **disabled on the dev box's system volume**, so `C:\WINDOW~1\SYSTEM~1` from `docs/11` Tests does not exist there. The case is real where short names are enabled, and is now probed rather than assumed. | `docs/11` Tests |
